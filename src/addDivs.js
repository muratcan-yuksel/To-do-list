...
const addDivs = () => {
  ...
  const test = new Date(TodoList[TodoList.length - 1].date);
  ...
  function initRemoveButton() {
    ...
    const index = TodoList.findIndex(
      (obj) => obj.title === title.textContent && obj.details === details.textContent,
    );
    // remove that index
    ...
    for (const project in projectArray) {
      ...
      const index = projectArray[project].findIndex(
        (obj) => obj.title === title.textContent && obj.details === details.textContent,
      );
      // remove that index
